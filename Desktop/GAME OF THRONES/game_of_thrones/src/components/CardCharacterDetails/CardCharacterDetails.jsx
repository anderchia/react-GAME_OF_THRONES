    ...
    <section>
      ...
      <figure className="b-boxdetail">
        ...
        <div className="b-boxdetailsmall">
          ...
          {character.allegiances && character.allegiances.map((item,i) => (
          <p className="b-tipo" key={i}>{item}</p>))}
        </div>
        <div className="b-boxdetailsmall">
       
          <h3>{t('description.part6')}</h3>
          <div className="timeline-container-small">
          {character.appearances && character.appearances.map((item,i) => (
          <p className="b-tipo" key={i}>{item}</p>))}
          </div>
        </div>
        <div className="b-boxdetailsmall">
        ...
        <div className="b-boxdetailsmall">
          ...
          {character.siblings && character.siblings.map((item,i) => (
          <p className="b-tipo" key={i}>{item}</p>))}
        </div>
        <div className="b-boxdetailsmall ">
          <h3>{t('description.part9')}</h3>
          <div className="timeline-container-small">
          {character.titles && character.titles.map((item,i) => (
          <p className="b-tipo" key={i}>{item}</p>))}
          </div>
        </div>
      </figure>
